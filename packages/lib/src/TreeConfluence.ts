...
async function ensurePageExists(
	...
	if (file.pageId) {
		try {
			...
			return {
				...
				lastUpdatedBy:
					contentById?.version?.by?.userKey ?? "NO ACCOUNT ID",
	...
	if (currentPage) {
		...
		return {
			...
			lastUpdatedBy: currentPage.version?.by?.userKey ?? "NO ACCOUNT ID",
		...
		return {
			...
			lastUpdatedBy: pageDetails.version?.by?.userKey ?? "NO ACCOUNT ID",
